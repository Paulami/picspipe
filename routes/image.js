...
module.exports = (upload) => {
        ...
        .post(upload.single('file'), (req, res, next) => {
            ...
            Image.findOne({ caption: req.body.caption })
                .then((image) => {
                    ...
                    newImage.save()
                    // I don't know if DB actually saved the file or not?
                        .then((image) => {
                            console.log(req.file.filename, ' saved!');
                            res.status(200).json({
                                success: true,
                                image,
                            });
    ...
    */
    imageRouter.route('/recent')
        ...
        imageRouter.route('/recent/:limit')
        .get((req, res, next) => {
            gfs.find().sort({ uploadDate:-1 }).limit(parseInt(req.params.limit, 10)).toArray((err, files) => { //todo try with Image
    ...
    */
    imageRouter.route('/files')
        .get((req, res, next) => {
            gfs.find().sort({uploadDate:-1}).limit(2).toArray((err, files) => {
                if (!files || files.length === 0) {
                    return res.status(200).json({
                        success: false,
                        message: 'No files available'
        ...
        .get((req, res, next) => {
            gfs.find({ filename: req.params.filename }).toArray((err, files) => {
                ...
                if (files[0].contentType === 'image/jpeg' || files[0].contentType === 'image/png' || files[0].contentType === 'image/svg+xml' || files[0].contentType === 'application/octet-stream') {
                    ...
                    gfs.openDownloadStreamByName(req.params.filename).pipe(res); // Look at this
                } else {
                    res.status(404).json({
                        err: 'Not an image',
                    });
